...
{	
	...
	/*$(".Popularity").click(function(){
		if($(".Popularity").find(':selected').text() == 'Between'){
			$(".PopularityFrom").show();
			$(".PopularityTo").show();
		}
		if($(".Popularity").find(':selected').text() == 'Below'){
			$(".PopularityFrom").hide();
			$(".PopularityTo").show();
		}else if($(".Popularity").find(':selected').text() == 'Above'){
			$(".PopularityFrom").show();
			$(".PopularityTo").hide();
		}
	});
	...
	$(".SellingPrice").click(function(){
		if($(".SellingPrice").find(':selected').text() == 'Between'){
			$(".SPFrom").show();
			$(".SPTo").show();
		}
		if($(".SellingPrice").find(':selected').text() == 'Below'){
			$(".SPFrom").hide();
			$(".SPTo").show();
		}else if($(".SellingPrice").find(':selected').text() == 'Above'){
			$(".SPFrom").show();
			$(".SPTo").hide();
		}
	});*/
	...
	$("div.action > span.processesubcat").click(function()
			...
			//alert("Please click on Apply Cashback BUTTON in view Summary Page to apply changes.");
			...
			$.ajax(
					{
						...
						success : function(data)
						{
							...
							{
								hideloader("div.action > div.report > div.bar");
								$("div.action > div.report > div.bar").html("<span>Internal Error! Sub Category rules not processed</span>");
							}
						}
					});	
			});
	...
	$("#updateSubcatBtn1").click(function()
	{
		...
		if(type == "OA")
		{
			...
			startTime=$("#subcatStartTimeField").val();
			endTime=$("#subcatEndTimeField").val();
		...
		$.ajax(
		{
			...
			dataType: 'html',
			url : '/PriceSimulator/ajax/updatesubcat.action',
			data : {type : type, filter : JSON.stringify(filter), ncgm : JSON.stringify(ncgm),cm : JSON.stringify(cm), gm : JSON.stringify(gm), cd : JSON.stringify(cd), md : JSON.stringify(md), nc : JSON.stringify(nc),
				cmpu : JSON.stringify(cmpu), nccmpu : JSON.stringify(nccmpu),idp : idp,wma : wma ,competitorList : compList,startTime : startTime, endTime : endTime, ruleID: ruleID},
			success : function(data)
			{
